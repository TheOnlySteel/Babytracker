<script lang="ts">
  ...
  import FeedPicker from './sheets/FeedPicker.svelte';
  import BottleSheet from './sheets/BottleSheet.svelte';
...
{#if ui.sheet}
  {#key ui.sheet}
    {#if ui.sheet.kind === 'feed-picker'}<FeedPicker />
    {:else if ui.sheet.kind === 'bottle'}<BottleSheet entry={ui.sheet.entry} />
